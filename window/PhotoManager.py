...
from window import Theme
from window.MainWindow import MainWindow
from window.SQLMultiThreadOK import SQLMultiThreadOK
...
class PhotoManager(App):
    ...
    def setup_database(self, restore=False):
        ...
        try:
            ...
            self.session.query(Folder).all()  # verify if database exist

        except Exception as e:
            raise Exception('Must create database before')
    ...
    # todo move folder with folder item
    ...
    def database_import_files(self):
        ...
        if self.screen_manager.current == 'screenDatabase':
            ...
            Clock.schedule_once(self.database_screen.create_treeview)
    ...
    def drag(self, drag_object, mode, position, image=None, offset=list([0, 0]), fullpath=''):
        ...
        if mode == 'end':
            self.main_layout.remove_widget(self.drag_image)
            self.screen_manager.current_screen.drop_widget(drag_object.photo, position, dropped_type='file', aspect=self.drag_image.image_ratio)
    ...
    def drag_treeview(self, drag_object, mode, position, offset=list([0, 0])):
        ...
        if mode == 'end':
            self.main_layout.remove_widget(self.drag_treenode)
            self.screen_manager.current_screen.drop_widget(drag_object, position, dropped_type=drag_object.droptype, aspect=1)
